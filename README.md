...
где h - шаг сетки, x_i+1 = x_i + h, k1 = f(x_i, y_i), k2 = f(x_i+1, y_i + h*k1) - вычисленный с помощью k1 наклон второго порядка на отрезке [x_i, x_i+1].

Для нахождения k2 используется метод Эйлера, где на отрезке [x_i, x_i+1] происходит движение из начальной точки (x_i, y_i) с шагом h*k1.
